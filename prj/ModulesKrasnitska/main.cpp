...
int main() {
    ...
    int count = 0;
    int bit = n & 1; // Check the value of the least significant bit

    if (bit == 1) { // Count the number of zeros if the least significant bit is 1
        while ((n & 1) == 0 && n > 0) {
            count++;
            n >>= 1; // Shift n to the right by 1 bit
        }
    } else { // Count the number of ones if the least significant bit is 0
        while ((n & 1) == 1) {
            count++;
            n >>= 1; // Shift n to the right by 1 bit
        }
    ...
    std::cout << "The number of " << (bit == 1 ? "zeros" : "ones") << " in N is " << count << std::endl;
